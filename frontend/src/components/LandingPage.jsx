...
export function LandingPage({ onAuth }) {
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white font-sans">
      ...
      {/* ── NAVBAR ── */}
      <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-8 h-14 sm:h-16
                      bg-[#0d1117]/90 backdrop-blur-md border-b border-white/[0.06]">
        <div className="flex items-center gap-2 sm:gap-3">
          <img src="/OrthoVita.png" alt="OrthoVita Logo" className="h-10 sm:h-12 w-auto object-contain rounded-lg" />
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={openLogin}
            className="text-xs sm:text-sm font-medium text-gray-400 border border-white/10 px-3 sm:px-4 py-1.5 sm:py-2
          ...
          <button
            onClick={openSignup}
            className="text-xs sm:text-sm font-semibold bg-cyan-400 text-gray-950 px-3 sm:px-4 py-1.5 sm:py-2
      ...
      {/* ── HERO ── */}
      <section className="flex flex-col items-center text-center px-4 sm:px-6 pt-16 sm:pt-24 pb-12 sm:pb-20">

        {/* Eyebrow badge */}
        <div className="flex items-center gap-2 text-cyan-400 text-[10px] sm:text-xs font-semibold
                        tracking-widest uppercase border border-cyan-400/25 bg-cyan-400/5
                        px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-6 sm:mb-8">
        ...
        <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold tracking-tight
                       leading-tight text-white max-w-3xl mb-4 sm:mb-5">
          Smart Rehabilitation &{' '}
          <span className="text-cyan-400">AI Physical Therapy</span>
        </h1>

        {/* Sub */}
        <p className="text-gray-400 text-sm sm:text-base lg:text-lg max-w-xl leading-relaxed mb-8 sm:mb-10">
        ...
        {/* CTAs */}
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-2 sm:gap-3 mb-12 sm:mb-16 w-full sm:w-auto">
          <button
            onClick={openSignup}
        ...
        <div className="flex divide-x divide-white/[0.07] border border-white/[0.07]
          ...
          {[
            ...
            <div key={label} className="flex-1 py-3 sm:py-4 text-center">
              <div className="text-cyan-400 text-lg sm:text-xl font-bold">{val}</div>
              <div className="text-gray-500 text-[10px] sm:text-xs mt-0.5 uppercase tracking-wide font-medium">
                {label}
              </div>
            </div>
          ))}
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-12 sm:py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5 sm:p-6
                         hover:bg-white/[0.055] hover:border-white/[0.12] transition-all"
            >
              <div className="text-2xl mb-3 sm:mb-4">{f.icon}</div>
              <h3 className="text-white font-semibold text-sm sm:text-base mb-2">{f.title}</h3>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-12 sm:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
          {[
            ...
            <div
              key={s.step}
              className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 sm:p-7"
            >
              <div className="text-cyan-400 text-xs font-bold uppercase tracking-widest mb-3 sm:mb-4">
                Step {s.step}
              </div>
              <h3 className="text-white font-semibold text-sm sm:text-base mb-2">{s.title}</h3>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 pb-12 sm:pb-20">
        <div className="bg-cyan-400/5 border border-cyan-400/20 rounded-2xl px-6 sm:px-8 py-10 sm:py-14 text-center">
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white tracking-tight mb-2 sm:mb-3">
          ...
          <button
            onClick={openSignup}
            className="bg-cyan-400 text-gray-950 font-semibold text-sm px-6 sm:px-8 py-3 sm:py-3.5
                       rounded-xl hover:bg-cyan-300 transition-all shadow-lg shadow-cyan-400/20"
          >
            Create Free Account →
          </button>
      ...
      {/* ── FOOTER ── */}
      <footer className="border-t border-white/[0.06] px-4 sm:px-8 py-5 sm:py-6 flex flex-col sm:flex-row items-center
                         justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <img src="/OrthoVita.png" alt="OrthoVita Logo" className="h-5 sm:h-6 w-auto object-contain rounded-md" />
          <span className="text-sm font-semibold">
            Ortho<span className="text-cyan-400">Vita</span>
          </span>
        </div>
        <span className="text-gray-600 text-[10px] sm:text-xs text-center">
